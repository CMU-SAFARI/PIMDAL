...
void init_buffer() {
    ...
    uint32_t seed = 0;
        ...
        std::uniform_int_distribution<int32_t> dist(1, 25);
...
#if PERF == 1
    std::ofstream file_cycles("saggr_cycles.csv", std::ofstream::app);
    std::ofstream file_time("saggr_time.csv", std::ofstream::app);
...
#else
    std::ofstream file_inst("saggr_inst.csv", std::ofstream::app);
