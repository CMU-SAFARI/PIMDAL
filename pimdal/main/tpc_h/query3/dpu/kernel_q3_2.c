...
__mram_noinit_keep int32_t o_shippriority[131072];
...
bool pred_o(key_ptr_t element) {
    bool res = element.key < dpu_args.o_date;
    return res;
}
...
int main() {
    ...
    */
    ...
    load_out(buffer_1, buffer_3, (uint32_t) o_orderdate, sel_results.t_count);
    barrier_wait(&barrier);

    load_out(buffer_1, buffer_4, (uint32_t) o_shippriority, sel_results.t_count);
    barrier_wait(&barrier);
