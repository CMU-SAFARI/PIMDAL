...
int main() {
    ...
    uint32_t buf_o_orderkey = (uint32_t) (buf_o_custkey + size*sizeof(key_ptr_t));
    uint32_t buf_o_orderdate = (uint32_t) (buf_o_orderkey + size*sizeof(key_ptr_t));
    uint32_t buf_o_shipprio = (uint32_t) (buf_o_orderdate + size*sizeof(key_ptr_t));
    uint32_t buffer_1 = (uint32_t) (buf_o_shipprio + size*sizeof(key_ptr_t));
    uint32_t buffer_2 = (uint32_t) (buffer_1 + size*sizeof(key_ptr_t));
    uint32_t sizes_1 = (uint32_t) (buffer_2 + size*sizeof(key_ptr_t));
    uint32_t sizes_2 = (uint32_t) (sizes_1 + (NR_DPU+1)*sizeof(uint64_t));
    ...
    */
    ...
    if (tasklet_id == 0) {
        ...
        merge_args.out_ptr = buf_o_custkey;
        merge_args.part_sizes = sizes_2;
    ...
    load_next(buf_o_custkey, buffer_1, buf_o_orderkey, merge_res.out_n);
    ...
    load_out(buf_c_custkey, buf_o_custkey, buf_o_orderdate, merge_res.out_n);
    barrier_wait(&barrier);

    load_out(buf_c_custkey, buf_o_orderkey, buf_o_shipprio, merge_res.out_n);
    barrier_wait(&barrier);
